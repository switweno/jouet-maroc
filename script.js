...
// إضافة وظيفة عرض شهادات العملاء
document.addEventListener('DOMContentLoaded', function() {
    // التأكد من وجود عناصر الشهادات
    const testimonialsWrapper = document.getElementById('testimonialsWrapper');
    const prevButton = document.querySelector('.testimonial-nav.prev-btn');
    const nextButton = document.querySelector('.testimonial-nav.next-btn');
    const dotsContainer = document.getElementById('testimonialDots');
    
    if (!testimonialsWrapper || !prevButton || !nextButton || !dotsContainer) return;
    ...
    let slidesToShow = 1; // العدد الافتراضي للشرائح المعروضة
    ...
    // تحديث عرض الشريحة بناءً على عدد الشرائح المعروضة
    function updateSlideWidth() {
        const containerWidth = testimonialsWrapper.parentElement.offsetWidth;
        slideWidth = containerWidth / slidesToShow;
        
        testimonialCards.forEach(card => {
            card.style.flex = `0 0 ${slideWidth - 40}px`; // مع مراعاة هوامش البطاقة
        });
    }
    ...
    // الانتقال إلى شريحة محددة - مع تصحيح اتجاه التحريك لـ LTR
    function goToSlide(index) {
        ...
        // تحريك الشرائح - استخدام القيمة السالبة لتحريك من اليسار إلى اليمين (LTR)
        testimonialsWrapper.style.transform = `translateX(${-currentIndex * slideWidth}px)`;
    ...
    // معالجة النقر على الأزرار - تم تبديل الوظائف لتتناسب مع اتجاه LTR
